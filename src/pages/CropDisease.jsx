...
export default function CropDisease() {
  ...
  const handleAnalyze = async () => {
    if (!image) return
    setAnalyzing(true)
    // Placeholder — model will be integrated here later
    await new Promise(r => setTimeout(r, 2000))
    setResult({ status: 'pending', message: 'AI model not yet connected. Upload your trained model to enable disease detection.' })
    setAnalyzing(false)
  }
  ...
  return (
    <div className="cd-page fade-in">
      <div className="cd-header">
        <div>
          <h1 className="dash-title"><Microscope size={22} style={{ display: 'inline', marginRight: 8 }} />Crop Disease Detection</h1>
          <p className="dash-sub">Mote Patil Sugarcane Farms · Upload a crop image for AI-powered disease analysis</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <div className="badge badge-yellow"><AlertTriangle size={11} /> Model Pending</div>
      ...
      <div className="cd-main">
        ...
        <div className="cd-upload-col">
          <div className="card">
            <div className="card-section-title" style={{ marginBottom: 16 }}><ImagePlus size={13} /> Upload Crop Image</div>
            ...
            {!imageUrl ? (
              ...
              <div className="cd-preview">
                <div className="cd-preview-img-wrap">
                  ...
                  <button className="cd-clear-btn" onClick={clearImage}><X size={14} /></button>
                </div>
          ...
          {result && (
            <div className="card cd-result-card" style={{ marginTop: 14 }}>
              <div className="card-section-title" style={{ marginBottom: 12 }}>
                {result.status === 'pending' ? <AlertTriangle size={13} /> : <CheckCircle size={13} />} Analysis Result
              </div>
              <div className="cd-result-pending">
                <div style={{ fontSize: 32, marginBottom: 10 }}>🤖</div>
                <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, color: 'var(--yellow)' }}>Model Not Connected</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>{result.message}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="cd-info-col">
          <div className="card">
            <div className="card-section-title" style={{ marginBottom: 16 }}>📚 Common Sugarcane Diseases</div>
